...
from mfb.utils.helpers import pretty_box_comment
...
def build_summary_text(loop: MFBLoop) -> str:
    ...
    lines = [
        ...
        "P(s) starts with AMP(s) -> SPK(s) -> AccV(s), with C(s) in the forward path and F(s) in the feedback path.",
        ...
        *sensor_lines,
        f"Feedback CorrHP(s): enabled={cfg.feedback_filters.offset_hp_enabled}, family={cfg.feedback_filters.offset_hp_family}, order={cfg.feedback_filters.offset_hp_order}, fc={cfg.feedback_filters.offset_hp_hz:.2f} Hz",
        f"Controller C(s): PID + CorrLP + LinkT + OutLP, with PID type={cfg.pid.controller_type}, mode={cfg.pid.mode}",
        f"CorrLP(s): enabled={cfg.pid.corr_lp_enabled}, family={cfg.pid.corr_lp_family}, order={cfg.pid.corr_lp_order}, fc={cfg.pid.corr_lp_hz:.1f} Hz",
